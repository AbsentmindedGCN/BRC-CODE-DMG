...
{
    internal sealed class VolumeMenu : MonoBehaviour
    {
        private static VolumeMenu _instance;
        ...
        {
            EnsureInstance();
            int cur = CodeDmgPlugin.ConfigSettings?.Volume?.Value ?? 80;
            _instance._selectedIndex = 0;
            for (int i = 0; i < Values.Length; i++)
                if (Values[i] == cur) { _instance._selectedIndex = i; break; }
        ...
        {
            if (_instance != null) return;
            var go = new GameObject("CodeDmgVolumeMenu");
            go.transform.SetParent(Core.Instance.UIManager.transform, false);
            _instance = go.AddComponent<VolumeMenu>();
            _instance.Build();
        }
        ...
        private void BuildRows()
        {
            ...
            _rows.Clear();
            int vis = Mathf.Min(Options.Length, MaxVisible);
        ...
        {
            _selectedIndex = (_selectedIndex + dir + Options.Length) % Options.Length;
            ScrollToSelected();
            UpdateHighlights();
            PlaySelectSfx();
        }
        ...
        {
            if (CodeDmgPlugin.ConfigSettings?.Volume != null)
                CodeDmgPlugin.ConfigSettings.Volume.Value = Values[_selectedIndex];
            PlayConfirmSfx();
        ...
        private void ScrollToSelected()
        {
            ...
            _windowStart = Mathf.Clamp(_windowStart, 0, Mathf.Max(0, Options.Length - vis));
            UpdateVisibleRows();
        }
        ...
        private void UpdateVisibleRows()
        {
            for (int i = 0; i < _rows.Count; i++)
            {
                ...
                bool active = idx < Options.Length;
